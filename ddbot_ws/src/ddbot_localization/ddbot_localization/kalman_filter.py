...
class KalmanFilter(Node):
    ...
    def __init__(self):
        ...
        self.odom_sub_ = self.create_subscription(Odometry, "ddbot_controller/odom_noisy", self.odomCallback, 10)
        self.imu_sub_ = self.create_subscription(Imu, "imu/out", self.imuCallback, 10)
        self.odom_pub_ = self.create_publisher(Odometry, "ddbot_controller/odom_kalman", 10)
        
        # Initially the robot has no idea about how fast it's going, so set a very large variance for the z-angular velocity
        self.mean_ = 0.0        # the mean value of the robot z-angular velocity
        ...
        self.last_angular_z_ = 0.0      # last estimation from wheel encoders
        self.motion_ = 0.0              # motion = difference between the angular velocity of the robot at two consecutive moments in time
